...
class info_parser:
    ...
    def gen_walk_data(self, wait_time = 2.5, vis_flag = False, velocity = 0.1, vel_interest = 0, hardware = False):
        self.setup(show_output = vis_flag, hardware=hardware)
        time.sleep(1)
        scale_factor = 0
    ...
    def find_worst_case_velocity(self, vel_interest = 0, scale = 10):
        ...
        optimizer = UCBOptimizer(objective = objective, bounds = bounds, B = 2,
            R = 0.4, delta = 1e-6, tolerance = 0.003, n_init_samples = 1,
            length_scale = 1)
...
if __name__ == '__main__':
    ...
    #optimzer = commander.compare_velocities()

    # vel = float(sys.argv[1])
    # commander.gen_walk_data(velocity = vel, vel_interest = 1)
    # commander.walking_robustness(des_vel = vel, vel_flag = 1, visualizer = True)
    # commander.animate_data(des_vel = vel, vel_flag = 0)
    #np.save('optimality/VX_X_sample.npy',optimality.X_sample)
    #np.save('optimality/VX_Y_sample.npy',optimality.Y_sample)
